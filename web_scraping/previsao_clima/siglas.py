...
try:
	...
	# 	print(str(cid[0]))
	for estado in sorted(aeroportos.keys()):
		for sigla in sorted(aeroportos[estado]):
			print(estado, sigla, aeroportos[estado][sigla])
			conexao.cursor.execute("insert into aeroportos(estado,sigla,cidade) values(%s,%s,%s)",
            					  (estado, sigla, aeroportos[estado][sigla]))
