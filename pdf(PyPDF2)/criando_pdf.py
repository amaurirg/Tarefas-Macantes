...
pdfFile1 = open('meetingminutes.pdf', 'rb')				#abre o 1º arquivo do tipo binário em modo leitura.
pdfFile2 = open('meetingminutes2.pdf', 'rb')			#abre o 2º arquivo do tipo binário em modo leitura.
...
pdf2Reader = PyPDF2.PdfFileReader(pdfFile2)				#obtém o objeto PdfFileReader para leitura em pdf2Reader para representar esse PDF.
...
for pageNum in range(pdf2Reader.numPages):				#percorre as páginas do arquivo.
	pageObj = pdf2Reader.getPage(pageNum)				#obtém o objeto PdfFileReader em pageObj.
	pdfWriter.addPage(pageObj)							#adiciona a página no objeto pdfWriter.
...
pdfFile2.close()										#fecha o arquivo